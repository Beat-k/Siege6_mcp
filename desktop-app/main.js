...
app.whenReady().then(() => {
  ...
  // Register global hotkey (Ctrl+Alt+F11)
  ...
  ipcMain.handle('get-maps', () => maps);
  ipcMain.handle('search-operator', (event, query) => {
    const results = Object.entries(operators)
      .filter(([name]) => name.toLowerCase().includes(query.toLowerCase()))
      .slice(0, 5);
    return results;
  });
  ipcMain.handle('search-map', (event, query) => {
    const results = Object.entries(maps)
      .filter(([name]) => name.toLowerCase().includes(query.toLowerCase()))
      .slice(0, 5);
    return results;
  });
  ...
  console.log('Press Ctrl+Alt+F11 to show/hide the reference window');
});
